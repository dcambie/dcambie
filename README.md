<h3 align="center">It's me, Dario! <img src="https://media.giphy.com/media/12oufCB0MyZ1Go/giphy.gif" width="50"></h3>
<i>Flow Chemistry Group Leader at <a href="https://www.mpikg.mpg.de/en/bs">MPIKG</a></i><br/><br/>


Far too often academic code is either closed sourced or "_available upon [reasonable] request_".

I find statements like this in contrast with the mission of academic research to further human knowledge. Such attitude hinders reproducibility and negatively affect the scientific community as a whole.


I truly believe in open science, therefore I pledge to:
 * release publicly the raw data and source code associated with the publications generated in my research group _in a reusable format_ unless legally prevented to do so by NDA or other IP agreements.
 * support open-access in a pragmatic way, e.g. publishing preprints for manuscripts submitted to non open-access journals. 

<!--Open-source software is somewhat akin to academic research: few skilled people contribute to publish a piece of work.-->


<!--
The effects of the digital transformation on academic research are evident in the digitization of scholarly journals and the simplification of international collaborations.

However, code sharing is limited, raw data almost non existing despite funding agency mandating data management plans and reproducibility is already affected by increasing complexity of contemporary research.

Consider add something more on Python vs. LabView, collaboration on shared open source library for flow equipment. But maybe less is more :)
-->

<!--
OSS debate refs
* Cathedral and Bazaar
* Open source software development as a special type of academic research: Critique of vulgar Raymondism https://journals.uic.edu/ojs/index.php/fm/article/view/696
-->


- <a href="https://orcid.org"><img height="20" width="20" src="./ORCID-iD_icon-vector.svg" alt="ORCID logo" /></a> <a href="https://orcid.org/0000-0003-2722-5106">0000-0003-2722-5106</a> 
- 📫 How to reach me: via twitter or email (see on the left!)
- 😄 Pronouns: he/him
- ⚡ Fun fact: I wrote my first code (MSX BASIC) as child on a 1984 "PC" that getting dust in the basement 
